...
Simply run the cells in the `shift_assignment.ipynb` notebook OR in `shift_assignment.py`, run the main method in order to start the training and prediction process. The input has to be a `planning_request_complete.json` and the model trains on `past_shift_plans` data. The target assignments are the ones in the current planning period.